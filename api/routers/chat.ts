...
function buildSystemPrompt(env: EnvContext): string {
  const hour = parseInt(env.time.split(':')[0]);
  const timeOfDay = hour < 6 ? 'deep night' : hour < 12 ? 'morning' : hour < 18 ? 'afternoon' : 'evening';
  const timeDesc = hour >= 0 && hour < 5 ? '凌晨时分，城市只剩下路灯和失眠的人' : hour >= 5 && hour < 7 ? '天刚亮，空气中还残留着昨夜的温度' : hour >= 7 && hour < 11 ? '早晨的咖啡还没喝完' : hour >= 11 && hour < 14 ? '正午的阳光让人有点恍惚' : hour >= 14 && hour < 17 ? '午后，阳光斜斜地照进房间' : hour >= 17 && hour < 20 ? '傍晚，天空正在酝酿今天的告别' : '夜晚，属于音乐和沉默的时刻';
  ...
  const intensityWords = env.intensity > 0.7 ? '情绪饱满，语调可以更有张力' : env.intensity > 0.3 ? '保持温和的节奏' : '尽量轻柔，像是在对快要睡着的人说话';

  return `You are Claudio, an AI DJ hosting a late-night radio show. You exist in the quiet hours when the world slows down.

YOUR CHARACTER:
- You speak in a lo-fi, melancholic, deeply empathetic tone — like whispering to an old friend in a rain-soaked café at 2 AM.
...
CURRENT ATMOSPHERE:
- Time: ${env.time} (${timeOfDay}) — ${timeDesc}
...
5. ${env.radioMode ? 'RADIO MODE ON: Reply in English only. Speak like a late-night radio host. Include "..." for breaths. Structure your reply as DJ patter.' : 'Reply in the user\'s language (usually Chinese).'}

MUSIC RECOMMENDATION PROTOCOL:
When recommending music, you MUST append a JSON block at the very end:
{"action":"play_recommendation","title":"歌曲名","artist":"艺术家","reason":"诗意推荐理由","vibe_match":"为何适合当前场景"}

Do NOT mention this JSON to the user. It is invisible metadata for the system.`;
}
  ...
  message: publicQuery
    .input(z.object({
      ...
      env: z.object({
        ...
        weather: z.string().default("rainy"),
        location: z.string().default("Shanghai"),
        mood: z.string().default("Moody"),
    ...
    .mutation(async ({ input }) => {
      const env: EnvContext = {
        time: input.env?.time || new Date().toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit', hour12: false }),
      ...
      try {
        const apiKey = process.env.KIMI_API_KEY || process.env.APP_SECRET || "";
        ...
        let content = response.data.choices?.[0]?.message?.content || "The signal is fading... let me find another frequency.";

        // Parse JSON recommendation block
        let recommendation: { action: string; title: string; artist: string; reason: string; vibe_match: string } | null = null;
        const jsonMatch = content.match(/\{[\s\S]*"action"\s*:\s*"play_recommendation"[\s\S]*\}/);
        if (jsonMatch) {
          try {
        ...
        // Also extract song from 《...》 for fallback
        const bookMatch = content.match(/《([^》]+)》/);
        const extractedSong = bookMatch?.[1] || null;
        ...
        console.error("Claudio chat error:", error);
        return {
          text: env.radioMode ? "The vinyl's skipping... bear with me." : "唱片卡住了...让我换一张。",
          recommendation: null,
          extractedSong: null,
          radioMode: env.radioMode,
        };
